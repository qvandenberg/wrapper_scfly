    ...
    def time_integrate(self, inputParameters): # inputParameters, i_start=None,i_end=None,t_start=None,t_end=None
        ...
        for i in range(self.i_start, self.i_end+1): # loop over intensity
            ...
            for j in range(self.t_start, self.t_end+1):
                data = np.loadtxt(os.path.join(data_in_path, "sp%03d%s" % (j, "i"+str(i))), skiprows=1)
                freqs = data[:,0]
...
# generalise to do the same for temperature, density, population
        ...
        np.append(surface_weights, surface_weights[-1])
        surface_weights = np.diff(S_interpolated,n=1)
        np.append(surface_weights, surface_weights[-1]) # duplicate last item to restore length
...
class extract:
    ...
    def temperature_density(self,inputParameters):
        if not os.path.isdir(self.basepath+"/processed_data/conditions"):
            call(["mkdir",self.basepath+"/processed_data/conditions"])
            conditions_logfile = open(self.basepath+"/processed_data/conditions/conditions.log",'w+a')
            conditions_logfile('Intensity folders considered for temperature-density evolution: \t i%s until i%s \n' % (str(self.i_start),str(self.i_end)))
            conditions_logfile('Time steps considered: \t %s until %s \n' %(str(self.t_start),str(self.t_end)))
        for i in range(self.i_start,self.i_end+1):
            ...
            trho_out = open(self.basepath+"/processed_data/conditions/trho_i%d.log" %(i),'w+a')
            trho_out.write("%s\t%s\t%s\t%s\n" %('Index','Time [fs]','Temperature [eV]','Density [/cc]'))
        # print data

        if os.path.isdir(self.basepath+"/processed_data/fscanweights"):
            # weigh temperatures together to effective temperature
